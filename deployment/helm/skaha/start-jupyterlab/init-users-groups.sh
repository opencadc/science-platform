...
USER_MAPPINGS=$(curl -SsL --header "accept: text/plain" --header "authorization: Bearer ${1}" "${2}")
...
GROUP_MAPPINGS=$(curl -SsL --header "accept: text/tab-separated-values" --header "authorization: Bearer ${1}" "${3}")
SAVEIFS=$IFS
IFS=$(echo -en "\n\b")
