...
assert_contains "${metrics_rbac_render}" '  - apiGroups: ["batch"]
    resources: ["jobs"]
    verbs: ["get", "list"]'
assert_contains "${metrics_rbac_render}" '  - apiGroups: [""]
    resources: ["pods"]
    verbs: ["get", "list"]'
assert_contains "${metrics_rbac_render}" '  - apiGroups: ["metrics.k8s.io"]
    resources: ["pods"]
    verbs: ["get", "list"]'

metrics_clusterrole_render="$(
    awk '/^kind: ClusterRole$/{capture=1} capture{print} capture && /^---$/{exit}' \
        <<<"${metrics_rbac_render}"
)"
assert_contains "${metrics_clusterrole_render}" 'resources: ["clusterqueues"]'
assert_contains "${metrics_clusterrole_render}" 'resourceNames:'
assert_contains "${metrics_clusterrole_render}" '- "cq-test"'
assert_contains "${metrics_clusterrole_render}" 'verbs: ["get"]'
assert_not_contains "${metrics_clusterrole_render}" '"list"'
assert_not_contains "${metrics_clusterrole_render}" '"watch"'
assert_not_contains "${metrics_clusterrole_render}" '"jobs"'
assert_not_contains "${metrics_clusterrole_render}" '"pods"'
