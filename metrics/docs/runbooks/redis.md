...
persistence, replication, backups, or upgrades.
...
error. It must not issue an uncoordinated fresh source read for every request.
After the serviceable deadline, a missing primary source produces HTTP 503.
...
| Report is 503 with `Retry-After` | No serviceable snapshot and no successful fill | Restore Redis/Kueue access, then retry |
| Report is 200 with stale cache provenance | A known complete snapshot remains serviceable | Restore the dependency; do not flush the cache |
...
Inspect `Ready`, `Cached`, `Age`, `Last-Modified`, and `Cache-Status`. Do not
paste a credential-bearing Redis URL into logs or tickets.
...
[`metadata-labels.md`](../metadata-labels.md) and
