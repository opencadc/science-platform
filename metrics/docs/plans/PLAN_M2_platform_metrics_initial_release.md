...
validation, cluster-backed Kueue fixtures, and an API contract that remains open
to future resource groups and resource names.

## Repository snapshot versus milestone target

**Shipped baseline (see `docs/plans/PLAN_M2_outcomes.md`):** the service accepts
`METRICS_PROVIDER_MODE` values `static` and `kueue` only; the former `live` mode
and in-app node/fallback providers are removed; Kueue connectivity and queue
configuration are validated during FastAPI lifespan before traffic is served.

**Remaining closure work** (docs, changelog, upgrade alias, Helm notes, parse
observability) is tracked in `docs/plans/PLAN_M2_post_review_feedback.md` so M2
can be declared closed without rereading multi-persona review threads.

The milestone closes when implementation, verification commands in outcomes, and
that closure checklist are complete—not when this plan reads plausible in
isolation.
...
process. The canonical service modes are `dev`, `staging`, `integration`, and
`production`, and milestone docs, charts, and operator guidance must use those
tokens consistently. In `Kueue` mode, startup validates Kueue connectivity,
Kueue installation, and the configured queue set. If validation fails, the
service shuts down rather than falling back to node-based or static behavior.

In `dev`, you assume a Kubernetes cluster is already running, provisioned, and
reachable through `kubectl`; Helm is also already installed and configured.
Cluster provisioning code is out of scope for this milestone and should be
removed from milestone-facing workflows. `staging`, `integration`, and
...
validation fails.
- Install Kueue in existing dev and CI-backed clusters via Helm for live
integration validation.
- Treat cluster provisioning as an external prerequisite rather than part of the
milestone workflow.
...
Operator-facing `KUEUE_METRICS_`* variables are **fill-only aliases** for empty
`METRICS_`* fields (see `docs/environment-contracts.md` and
`Settings._apply_operator_kueue_env_aliases` in `metrics/src/metrics/config.py`).
Keep this table stable across charts, docs, and tests.

- `KUEUE_METRICS_URL` fills `kube_api_url` / `METRICS_KUBE_API_URL` when unset
- `KUEUE_METRICS_CLUSTER_QUEUES` fills `kueue_cluster_queues` when unset
- `KUEUE_METRICS_COHORT` fills `kueue_cohort` when unset
- `METRICS_CACHE_TTL_SECONDS` defaults to **300** seconds for this milestone
(platform Kueue reads); charts may still override
...
`kueue.x-k8s.io/v1beta2` API surface for this milestone.
- Align the default platform cache TTL with the milestone default: 300 seconds
for `METRICS_CACHE_TTL_SECONDS` in `Settings` for Kueue-backed platform reads.
...
- **Existing-cluster dev contract:** `dev` assumes a cluster exists before the
...
The default meaning of platform `allocated` (`**flavorsUsage` total + borrowed**,
summed over configured queues; cohort object does not contribute usage rows) is
recorded with evidence in `docs/plans/PLAN_M2_outcomes.md`. Future contract flags
or alternate semantics belong to a later milestone unless regression data
requires a revision.
...
1. **Kueue environment bring-up**
  - Install Kueue `0.17.0+` via Helm into the already running dev or CI test
   cluster.
  - Assume `kubectl` already reaches the intended cluster context and Helm is
   already configured for it.
  - Remove cluster provisioning steps from milestone workflows; the cluster must
   exist before the bring-up script runs.
  - Seed `cq-proton`, `cq-neutron`, and `cq-electron` in `cohort-atom`.
  - Use one general-purpose bring-up script that installs Kueue via Helm and
   applies the shared fixtures before the metrics service starts.
...
4. **Platform contract design**
  ...
  - `allocated` semantics are locked per `PLAN_M2_outcomes.md` (`flavorsUsage`).
...
- Validate that the general-purpose bring-up script installs Kueue `0.17.0+`
via Helm and applies the shared fixtures without provisioning the cluster.
...
schema changes.
- Confirm the `allocated` contract matches `PLAN_M2_outcomes.md` (`flavorsUsage`
total + borrowed) against live queue state when changing aggregation code.
...
can describe different live states. Mitigate drift by treating
`PLAN_M2_outcomes.md` as the authoritative `allocated` definition and adding
tests when Kueue exposes new status fields.
...
- **Provisioning drift:** Old Minikube-only or cluster-creation steps can creep
back into the docs and scripts. Mitigate this by keeping the dev contract
explicit: the cluster already exists, and the milestone only installs Kueue,
fixtures, and the service.
...
- Kueue `0.17.0+` installation is automated through one general-purpose Helm
...
schema changes.
- The public meaning of `allocated` matches `docs/plans/PLAN_M2_outcomes.md`
(`flavorsUsage` total + borrowed) and regression tests cover it.
...
- Required gates pass.
- Post-review closure tasks in `docs/plans/PLAN_M2_post_review_feedback.md` are
done
or explicitly deferred with rationale (changelog, env precedence docs, `live`
alias policy, quantity parse logging, RBAC verb scope, operator README note).
