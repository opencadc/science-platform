...
public class SessionUtil {
    ...
    static void verifySession(final Session session, final String expectedSessionType, final String expectedName) {
        final String sessionType = session.getType();
        Assert.assertNotNull("no session type", sessionType);

        if (sessionType.equals(expectedSessionType)) {
            final String connectURL = session.getConnectURL();

            Assert.assertNotNull("no session ID", session.getId());
            Assert.assertEquals("wrong session name", expectedName, session.getName());
            Assert.assertNotNull("missing connect URL", connectURL);
