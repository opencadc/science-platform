# System ADR index

Cross-context decisions for the science-platform monorepo (Skaha + Metrics).

| ADR | Title |
| --- | --- |
| [0001](0001-platform-stats-integration-boundary.md) | Platform stats integration boundary |
| [0002](0002-co-deployed-metrics-with-skaha.md) | Co-deployed Metrics backend with Skaha |
| [0003](0003-metrics-staging-gitops-deferred-until-production-stabilization.md) | Metrics staging GitOps deferred until production stabilization |

Context glossaries: [`../../CONTEXT-MAP.md`](../../CONTEXT-MAP.md).

Metrics-specific ADRs: [`../../metrics/docs/adr/`](../../metrics/docs/adr/).

Skaha-specific ADRs: [`../../skaha/docs/adr/`](../../skaha/docs/adr/).
