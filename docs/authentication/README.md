...
**Example**:
```
$ export BASIC_AUTH_TOKEN = `echo "$CLIENT_ID:$CLIENT_SECRET" | base64`
$ curl --header "authorization: Basic $BASIC_AUTH_TOKEN" -d "grant_type=authorization_code" -d "code=sdfue887hdyr" -d "client_id=$CLIENT_ID" -d "client_secret=$CLIENT_SECRET" https://ska-iam.stfc.ac.uk/token
```
